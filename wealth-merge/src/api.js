...
export async function submitToLMS({ name, mobile, email, score, summaryDtls = 'Wealth Merge Lead' }) {
...
export async function updateLeadNew(leadNo, { name, mobile, date, time, remarks }) {
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval5: time || '',
      stringval6: remarks || 'Slot Booking via Wealth Merge',
...
export const LEAD_NO_KEY = 'wealthMergeLeadNo';
