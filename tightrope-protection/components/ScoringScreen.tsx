...
const ScoringScreen: React.FC<Props> = ({ result, playerName, playerMobile, onPlayAgain }) => {
  ...
  const handleShare = async () => {
    ...
    const shareText = `Hi! I just scored ${result.score}% in Bajaj Tightrope Protection! Balance your life and protect your future! Play here:`;
    ...
    <div className="screen-scroll flex flex-col justify-between px-6 py-6" style={{ background: 'linear-gradient(185deg, #030F26 0%, #08224F 50%, #030F26 100%)' }}>
      ...
      <div className="mx-1 my-4 p-5 bg-white/5 border border-white/10 rounded-[1.5rem] shadow-xl backdrop-blur-md flex flex-col items-center">
        ...
        <div className="grid grid-cols-3 gap-2 w-full mt-4 pt-4 border-t border-white/5 text-center">
          <div>
            <span className="text-[9px] font-black text-blue-200/50 uppercase tracking-wider block">Distance</span>
            <span className="text-sm font-extrabold text-white">{result.distance}m</span>
          </div>
          <div>
            <span className="text-[9px] font-black text-blue-200/50 uppercase tracking-wider block">Savings</span>
            <span className="text-sm font-extrabold text-green-400">₹{result.coins * 100}</span>
          </div>
          <div>
            <span className="text-[9px] font-black text-blue-200/50 uppercase tracking-wider block">Shields</span>
            <span className="text-sm font-extrabold text-[#00AEEF]">{result.shieldHits}</span>
          </div>
        </div>
      </div>
