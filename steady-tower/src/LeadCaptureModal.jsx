...
// Copied from the life-goals-bubble-shooter gold standard; copy restyled for Steady Tower.
...
export default function LeadCaptureModal({ score, onSubmitted }) {
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // No email key: api.js already sends `email_id: email || ''`, so omitting
      // it keeps the LMS payload shape identical.
      const result = await submitToLMS({
        name: name.trim(),
        mobile,
        score,
        summaryDtls: 'Steady Tower - Post Game Lead',
      });
