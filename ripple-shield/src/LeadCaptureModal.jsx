...
// Copied from the life-goals-bubble-shooter gold standard; copy restyled for Ripple Shield.
...
export default function LeadCaptureModal({ score, onSubmitted }) {
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // `email` is deliberately omitted: api.js already sends `email_id: email || ''`,
      // so the LMS payload shape is unchanged and the field is gone from the form.
      const result = await submitToLMS({
        name: name.trim(),
        mobile,
        score,
        summaryDtls: 'Ripple Shield - Post Game Lead',
      });
