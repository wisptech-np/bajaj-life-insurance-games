// LMS integration for Safe Stride Balancer
...
export async function submitToLMS(data) {
  ...
  const gameID = sessionStorage.getItem('gamification_gameId') || 'GAME_036'; // Unique ID for Safe Stride Balancer
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Safe Stride Balancer Lead"}`,
...
export async function updateLeadNew(leadNo, data) {
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || "Slot Booking via Safe Stride Balancer",
