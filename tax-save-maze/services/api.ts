...
export const submitToLMS = async (data: LMSLeadData): Promise<{ success: boolean; data?: any; error?: string }> => {
    ...
    const gameID = sessionStorage.getItem('gamification_gameId') || 'GAME_TAX_MAZE';
    ...
    const payload = {
        ...
        userId: userId,
        gameID: gameID,
        remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Tax Save Maze Lead"}`,
...
export function extractLeadNo(result: any): string | null {
...
export const LEAD_NO_KEY = 'taxSaveMazeLeadNo';
