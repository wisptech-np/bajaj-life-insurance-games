...
const EnterDetailsScreen: React.FC<Props> = ({ score, onSubmit }) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingLeadNo = sessionStorage.getItem('coverageArcherLeadNo') || 
      ...
      if (!existingLeadNo) {
        ...
        const response = await submitToLMS({
          ...
          email_id: email.trim(),
          score: score,
          summary_dtls: 'Coverage Archer Lead Submission'
        });
        
        if (response.success && response.data?.LeadNo) {
          // Store generated lead number in session
          sessionStorage.setItem('coverageArcherLeadNo', response.data.LeadNo);
