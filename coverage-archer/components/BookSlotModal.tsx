...
const BookSlotModal: React.FC<Props> = ({ name, mobile, onClose, onBookSuccess }) => {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const leadNo = sessionStorage.getItem('coverageArcherLeadNo') || 
      ...
      if (leadNo) {
        ...
        const res = await updateLeadNew(leadNo, {
          ...
          date: formattedDate,
          time: selectedSlot,
          remarks: `Appointment via Coverage Archer | Slot: ${selectedSlot}`
        });
        ...
        if (res.success) {
          if (res.data?.LeadNo) {
            sessionStorage.setItem('coverageArcherLeadNo', res.data.LeadNo);
