...
export async function submitToLMS(data) {
  ...
  const gameID = sessionStorage.getItem('gamification_gameId') || 'GAME_035';
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Wealth Miner Lead"}`,
...
export async function updateLeadNew(leadNo, data) {
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || "Slot Booking via Wealth Miner",
