...
export async function submitToLMS(data) {
  ...
  const gameID = sessionStorage.getItem('gamification_gameId') || 'edurise-jumper';
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | EduRise Jumper Lead`,
...
export async function updateLeadNew(leadNo, data) {
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || "Slot Booking via EduRise Jumper",
...
export const LEAD_NO_KEY = 'eduriseJumperLeadNo';
